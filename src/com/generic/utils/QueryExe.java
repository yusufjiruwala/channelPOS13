...
public class QueryExe {
    ...
    public void setParaValue(String paraname, Object val) throws SQLException {
        ...
        if (val instanceof dataCell) {
            mapParameters.get(paraname).setValueType(Parameter.DATA_TYPE_STRING);
            mapParameters.get(paraname.toUpperCase()).setValue(((dataCell) val).getValue());
        }
        if (val instanceof JTextComponent) {
            mapParameters.get(paraname).setValueType(Parameter.DATA_TYPE_STRING);
            mapParameters.get(paraname.toUpperCase()).setValue(((JTextComponent) val).getText());
        }
